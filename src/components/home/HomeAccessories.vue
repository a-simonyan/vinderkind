<template>
  <div class="bg-lavender">
    <div class="container flex flex-col justify-center items-center pt-[18px] pb-[46px]">
      ...
      <div v-if="accessories.length" class="pt-7 max-w-[814px] w-full pl-0 px-5 sm:px-5 lg:px-0">
        <SharedSwiper :data="accessories" :points="points">
          <template #default="{ index, content }">
            <CustomCart :data="content" :key="index" :allData="accessories" />
          </template>
        </SharedSwiper>
      </div>
    </div>
